...
app.post('/analyze', (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Analysis failed but server is running' });
  }
});
...
app.post('/scan', async (req, res) => {
  try {
    ...
    const productImages = allImgTags.filter(
      (t) => /product|cdn\.shopify|\/files\//i.test(t) && !/logo|icon|badge|avatar|sprite/i.test(t)
    ).length;
    const imagesWithoutAlt = allImgTags.filter((t) => !/alt=["'][^"']+["']/i.test(t)).length;
    ...
    if (!title) {
      ...
      score -= 5;
      issues.push({ issue: 'Page title too short (under 30 chars)', severity: 'Medium' });
      suggestions.push('Expand your page title to 50–60 characters for better SEO.');
    } else if (title.length > 70) {
      score -= 5;
      issues.push({ issue: 'Page title too long (over 70 chars)', severity: 'Low' });
    ...
    if (!hasOgTitle || !hasOgImage || !hasOgDescription) {
      ...
      suggestions.push('Add og:title, og:description, and og:image for better social sharing.');
    }
    ...
    if (h1Count === 0) {
      ...
      score -= 4;
      issues.push({ issue: `Multiple H1 tags found (${h1Count})`, severity: 'Medium' });
      suggestions.push('Use only one H1 per page.');
    }

    if (totalImages > 40) {
      score -= 8;
      issues.push({ issue: `Too many images on page (${totalImages})`, severity: 'High' });
      suggestions.push('Reduce the number of images on the homepage to improve speed.');
    } else if (totalImages > 20) {
      score -= 4;
      issues.push({ issue: `High image count (${totalImages})`, severity: 'Medium' });
      suggestions.push('Consider lazy-loading images to improve initial page load.');
    }
    ...
    if (pageSize > 500000) {
      score -= 10;
      issues.push({ issue: `Heavy page size (${(pageSize / 1024).toFixed(0)} KB)`, severity: 'High' });
      suggestions.push('Minify HTML/CSS/JS and compress assets to reduce page weight.');
    } else if (pageSize > 200000) {
      score -= 5;
      issues.push({ issue: `Large page size (${(pageSize / 1024).toFixed(0)} KB)`, severity: 'Medium' });
      suggestions.push('Aim for a page size under 200 KB for faster load times.');
    }

    if (responseTime > 3000) {
      score -= 10;
      issues.push({ issue: `Slow server response (${responseTime}ms)`, severity: 'High' });
      suggestions.push('Improve server response time — aim for under 1 second.');
    } else if (responseTime > 1500) {
      score -= 5;
      issues.push({ issue: `Moderate response time (${responseTime}ms)`, severity: 'Medium' });
    ...
    if (!url.startsWith('https://')) {
      ...
      suggestions.push('Enable HTTPS — it is a Google ranking factor and builds customer trust.');
    }
    ...
    res.json({
      ...
      images: { total: totalImages, productImages, missingAlt: imagesWithoutAlt },
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({ success: false, error: 'Something went wrong, but server is running' });
});
